    ...
    if ($email) {
        try {
            ...
            if ($row) {
                ...
                $upd = $conn->prepare("UPDATE appconfig SET saldo = :novoSaldo WHERE email = :email");
                $upd->bindParam(":novoSaldo", $novoSaldo);
...
if ($email) {
    try {
        $conn = getConnection();
        $stmt = $conn->prepare("SELECT percas FROM appconfig WHERE email = :email");
        $stmt->bindParam(":email", $email);
        $stmt->execute();
        ...
        if ($row1) {
            ...
            $upd = $conn->prepare("UPDATE appconfig SET percas = :percas WHERE email = :email");
            $upd->bindParam(":percas", $percas);
            $upd->bindParam(":email", $email);
            $upd->execute();
        }
    } catch (PDOException $e) {
