...
try {
    $conn = getConnection();
    $stmt = $conn->query("SELECT nome_unico, nome_um, nome_dois, cpa, revenue_share FROM app LIMIT 1");
...
{
    $stmt = $conn->prepare("SELECT id FROM appconfig WHERE id = :id");
...
{
    $stmt = $conn->prepare("SELECT email FROM appconfig WHERE email = :email");
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (!filter_var($email, FILTER_VALIDATE_EMAIL)) {
        ...
        $getNextIdStmt = $conn->query("SELECT COALESCE(MAX(CAST(id AS INTEGER)), 0) AS max_id FROM appconfig");
        $nextIdRow = $getNextIdStmt ? $getNextIdStmt->fetch(PDO::FETCH_ASSOC) : null;
        $nextId = $nextIdRow ? (int)$nextIdRow["max_id"] + 1 : 1;
        ...
        $insertQuery = "INSERT INTO appconfig (id, cpa, email, senha, telefone, saldo, lead_aff, linkafiliado, indicados, plano, saldo_comissao, data_cadastro, afiliado) 
                        VALUES (:id, :cpa, :email, :senha, :telefone, :saldo, :lead_aff, :linkafiliado, '0', :plano, :saldo_comissao, :data_cadastro, :afiliado)";
        ...
        if ($stmt) {
            ...
            $stmt->bindParam(":lead_aff", $leadAff);
            $stmt->bindParam(":linkafiliado", $linkAfiliado);
            $stmt->bindParam(":plano", $plano);
            $stmt->bindParam(":saldo_comissao", $saldo_comissao);
            $stmt->bindParam(":data_cadastro", $dataCadastroFormatada);
            $stmt->bindParam(":afiliado", $afiliado);
            if ($stmt->execute()) {
            ...
            $errorMessage = "Erro ao preparar a consulta.";
        }
    }
}
?>
