...
try {
    ...
    $client_id = "";
    $client_secret = "";
    $stmt = $conn->query("SELECT client_id, client_secret FROM gateway");
    $row = $stmt->fetch(PDO::FETCH_ASSOC);
    if ($row) {
...
try {
    ...
    $stmt = $conn->prepare("SELECT * FROM appconfig WHERE email = :email AND (jogoteste IS NULL OR jogoteste != '1')");
    $stmt->bindParam(":email", $email);
    $stmt->execute();
    if ($stmt->rowCount() > 0) {
        $upd = $conn->prepare("UPDATE appconfig SET jogoteste = '1' WHERE email = :email");
...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    if ($res["response"] === "OK") {
        ...
        try {
            ...
            $stmt = $conn->prepare("INSERT INTO confirmar_deposito (email, valor, externalreference, status, data) VALUES (:email, :valor, :extref, :status, :data)");
            $stmt->bindParam(":email", $email);
            $stmt->bindParam(":valor", $form["value"]);
            $stmt->bindParam(":extref", $res["idTransaction"]);
            $status = "WAITING_FOR_APPROVAL";
            $stmt->bindParam(":status", $status);
            $stmt->bindParam(":data", $userDate);
            $stmt->execute();
        } catch (Exception $ex) {
            var_dump($ex);
...
try {
    $conn = getConnection();
    $stmt = $conn->query("SELECT deposito_min FROM app LIMIT 1");
    $row = $stmt->fetch(PDO::FETCH_ASSOC);
    $depositoMinimo = $row ? $row["deposito_min"] : 2;
} catch (PDOException $e) {
    $depositoMinimo = 2;
}
?>
