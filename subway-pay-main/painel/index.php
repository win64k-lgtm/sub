...
// Carregar nome da aplicação usando PDO PostgreSQL
try {
    $conn = getConnection();
    ...
    die("Erro ao carregar configuração do app: " . $e->getMessage());
}
?>
...
try {
    $conn = getConnection();
    if (isset($_SESSION["email"])) {
        $stmt = $conn->query("SELECT dificuldade_jogo FROM app LIMIT 1");
        $row = $stmt->fetch(PDO::FETCH_ASSOC);
        if ($row) {
    ...
    die("Erro ao carregar dificuldade do jogo: " . $e->getMessage());
}
?>
