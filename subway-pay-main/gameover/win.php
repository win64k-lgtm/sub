...
if ($email && isset($_POST["msg"])) {
    ...
    try {
        ...
        if ($row) {
            ...
            $updGanhos = $conn->prepare("UPDATE appconfig SET ganhos = ganhos + :valor WHERE email = :email");
            $updGanhos->bindParam(":valor", $valor);
            $updGanhos->bindParam(":email", $email);
            $updGanhos->execute();
...
try {
    ...
    if (isset($_SESSION["email"])) {
        ...
        $stmtSaldo = $conn->prepare("SELECT saldo FROM appconfig WHERE email = :email");
        $stmtSaldo->bindParam(":email", $email);
        $stmtSaldo->execute();
