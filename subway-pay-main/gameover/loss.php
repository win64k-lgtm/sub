...
try {
    ...
    $updPercas = $conn->prepare("UPDATE appconfig SET percas = percas + :bet WHERE email = :email");
    $updPercas->bindParam(":bet", $bet);
    $updPercas->bindParam(":email", $email);
    $updPercas->execute();

    $stmtLead = $conn->prepare("SELECT lead_aff FROM appconfig WHERE email = :email");
    $stmtLead->bindParam(":email", $email);
    $stmtLead->execute();
    $leadAff = $stmtLead->fetchColumn();

    if ($leadAff) {
        $stmtPlano = $conn->prepare("SELECT plano FROM appconfig WHERE id = :id");
        $stmtPlano->bindParam(":id", $leadAff);
        $stmtPlano->execute();
        $plano = $stmtPlano->fetchColumn();

        if (is_numeric($plano) && $plano > 0) {
            $addValue = $bet * ($plano / 100);
            $updComissao = $conn->prepare("UPDATE appconfig SET saldo_comissao = saldo_comissao + :addValue WHERE id = :leadAff");
            $updComissao->bindParam(":addValue", $addValue);
            $updComissao->bindParam(":leadAff", $leadAff);
            $updComissao->execute();
        }
    }

    $novoSaldo = $saldoAtual - $bet;
    $updSaldo = $conn->prepare("UPDATE appconfig SET saldo = :novoSaldo WHERE email = :email");
    $updSaldo->bindParam(":novoSaldo", $novoSaldo);
    $updSaldo->bindParam(":email", $email);
    $updSaldo->execute();
