...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    try {
        $conn = getConnection(); // PDO PostgreSQL
        ...
        // Buscar saldo atual
        $stmt_saldo = $conn->prepare("SELECT saldo FROM appconfig WHERE email = :email");
        $stmt_saldo->execute([':email' => $session_email]);
        ...
        if ($saldo !== false && $saldo >= $valor && $valor > 0) {
            ...
            // Atualizar saldo
            $stmt_update_saldo = $conn->prepare(
                "UPDATE appconfig SET saldo = :novo_saldo WHERE email = :email"
            );
            ...
            $stmt_insert_saque = $conn->prepare(
                "INSERT INTO saques (email, externalreference, valor, status) 
                 VALUES (:email, :external_reference, :valor, :status)"
            );
            ...
            if ($stmt_update_saldo->rowCount() > 0 && $stmt_insert_saque->rowCount() > 0) {
                $conn->commit();
                // Redirecionar para a página de sucesso com parâmetros da URL
                header(
                    "Location: saque_realizado.php?withdrawName=" . urlencode($_POST['withdrawName']) .
                    "&withdrawCPF=" . urlencode($_POST['withdrawCPF']) .
                    "&withdrawValue=" . urlencode($_POST['valor'])
                );
                exit();
            } else {
                $conn->rollBack();
        ...
        echo "Erro ao processar o saque: " . $e->getMessage();
        exit;
    }
}
